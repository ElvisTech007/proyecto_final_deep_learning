...
# ABLACIÓN: Solo usamos bandas lentas
BANDS_LOW = {
    'Delta': (0.5, 4), 
    'Theta': (4, 8), 
    'Alpha': (8, 13)
    # Beta y Gamma ELIMINADAS
}
def extraer_features_ablacion(path_npz):
    print("--- EXTRAYENDO SOLO BANDAS LENTAS (DELTA, THETA, ALFA) ---")
    ...
    # Ahora son 3 bandas * 19 canales = 57 features (antes eran 95)
    X_features = np.zeros((n_ventanas, 19 * 3), dtype=np.float32)
    ...
    for i in range(0, n_ventanas, BATCH_SIZE):
        ...
        for j in range(len(batch)):
            ...
            for c in range(n_canales):
                psd_c = psd[j, :, c]
                # Calculamos potencia total igual (0.5 a 45) para mantener la referencia
                idx_total = np.logical_and(freqs >= 0.5, freqs <= 45)
                total_power = simpson(psd_c[idx_total], dx=freqs[1]-freqs[0]) or 1.0
                
                # Solo iteramos las bandas bajas
                for banda in BANDS_LOW.values():
                    idx_band = np.logical_and(freqs >= banda[0], freqs <= banda[1])
...
def main():
    ...
    X_feats, y, groups = extraer_features_ablacion('datos_eeg_procesados.npz')
    ...
    plt.savefig('resultado_final_modificado.png')
